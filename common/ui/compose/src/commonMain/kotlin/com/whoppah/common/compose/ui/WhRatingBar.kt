...
import com.whoppah.common.compose.icons.state.Star
import com.whoppah.common.compose.icons.state.StarOutlined
...
fun WhRatingBar(
    ...
    rating: Float,
    itemCount: Int = 5,
    imageVectorEmpty: ImageVector = WhIcons.State.StarOutlined,
    imageVectorFilled: ImageVector = WhIcons.State.Star,
    tintEmpty: Color = WhoppahPalette.Burst500,
    tintFilled: Color = WhoppahPalette.Burst500,
    ...
    val colorFilterFilled: ColorFilter = remember(tintFilled) { ColorFilter.tint(tintFilled) }
    ...
    Box(modifier) {
        ...
        Box(
            modifier = Modifier
                ...
                .width(totalWidth)
                .height(height)
                .drawBehind { block(animatableRating.value, spacePx) }
        )
    }
}
...
private fun DrawScope.drawWithLayer(block: DrawScope.() -> Unit) {
    // 1. Calculate the bounds of the layer (usually the whole drawing area)
    val bounds = Rect(0f, 0f, size.width, size.height)

    // 2. Access the Compose Canvas wrapper (not nativeCanvas)
    val canvas = drawContext.canvas

    // 3. Save the layer using pure Compose API
