...
/**
 * Represents the status packet (Version 1) received from the device.
 * Contains both sensor data (PV - Process Values) and device settings (SP - Set Points).
 *
 ...
 * @property pvCO2 Current CO2 concentration reading (scaled, max 5000, min 400).
 ...
 * @property output Output control signals (as recorded in 74HC595D).
 ...
 * @property gearbox коэффициент передачи от асинхрон.дв. к вентилятору (маска 0001 1111).
...
data class StatusPacketV1(
    ...
    val spRh0: Int = 0,      // 1 байт ind=24 ПОДСТРОЙКА HIH-5030
    val spRh1: Int = 0,      // 1 байт ind=25 Уставка влажности Датчик HIH-5030
    ...
    val minRun: Float = 0f,     // 1 байт ind=30 импульсное управление насосом увлажнителя x/10
    ...
    val gearbox: Int = 0,       // 1 байт ind=54; коэффициент передачи от асинхрон.дв. к вентилятору (маска 0001 1111)
